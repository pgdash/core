...
impl<'a, C: DatabaseClient> PostgresScanner<'a, C> {
    ...
    pub async fn scan(&self, database_name: &str) -> Result<Database, String> {
        ...
        for (schema_name, table_name, oid) in schemas_found {
            ...
            let s_name = schema.name.clone();

            info!("running scan tasks for table: {}", table_name);
            let (col_res, con_res, idx_res, trig_res) = tokio::join!(
                self.scan_columns(&s_name, &table_name),
                self.scan_constraints(&s_name, &table_name),
                self.scan_indexes(&s_name, &table_name),
                self.scan_triggers(&s_name, &table_name),
            );
        ...
        for row in rows {
            ...
            let entry = constraint_map
                ...
                .or_insert_with(|| ConstraintGroup {
                    ...
                    foreign_schema,
                    foreign_table,
                    update_action: update_rule.as_deref().map(map_referential_action),
                    delete_action: delete_rule.as_deref().map(map_referential_action),
    ...
    async fn scan_functions(
        ...
        for row in rows {
            ...
            if let (Some(s_name), Some(r_name)) = (schema_name, routine_name) {
                ...
                let s_name_ref = schema.name.clone();
                ...
                let param_rows = self.client.query(param_query, &[&s_name_ref, &r_name]).await?;
