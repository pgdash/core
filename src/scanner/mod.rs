...
impl<'a, C: DatabaseClient> PostgresScanner<'a, C> {
    ...
    pub async fn scan(&self, database_name: &str) -> Result<Database, String> {
        ...
        for (schema_name, table_name, oid) in schemas_found {
            // Check-then-insert pattern avoids allocating a new String for `schema_name.clone()`
            // when the schema already exists in the map, unlike the `.entry()` API.
        ...
        match self.client.query(views_query, &[]).await {
            Ok(view_rows) => {
                for row in view_rows {
                    ...
                    // Zero-cost abstraction: check-then-insert avoids unnecessary String clones
                    #[allow(clippy::map_entry)]
                    if !schemas_map.contains_key(&schema_name) {
                        schemas_map.insert(
                            schema_name.clone(),
    ...
    async fn scan_constraints(
        ...
        for row in rows {
            ...
            let entry = constraint_map.get_mut(&name).unwrap();
