...
* **HashMap Entry Allocations**: When inserting owned `String`s into a `HashMap`, `.entry(string.clone()).or_insert_with(...)` causes unnecessary upfront heap allocations on cache hits. Replacing this pattern with `.entry(string_value).or_insert_with_key(|k| ...)` transfers ownership, saving a redundant `.clone()` and avoiding double lookups while cleanly passing the Clippy `map_entry` lint. You can subsequently re-borrow from the stored structure reference if the consumed value is needed later in the scope.
